...
import { ArrowLeft, Check, Copy, Heart, Link, LockKeyhole, LogOut, Mail, Sparkles, Users } from 'lucide-react'
...
export function SignalLamp({ onBack }: { onBack: () => void }) {
  ...
  const sendLink = async () => {
    ...
    setAuthNotice(error ? error.message : '验证码登录链接已发送，请在邮箱中打开。')
  }
  ...
  if (!session) return <main className="signalPage"><nav><button className="back" onClick={onBack}><ArrowLeft size={18} /> 返回词典</button><div className="brand"><Heart fill="#ca5a62" /> 恋爱信号灯</div><span className="privacy"><LockKeyhole size={15} /> 私密二人空间</span></nav><section className="authBox"><Mail size={30}/><h1>先登录，再点亮信号灯</h1><p>用邮箱验证码登录；无需设置密码。</p><input type="email" placeholder="你的邮箱" value={email} onChange={e => setEmail(e.target.value)} /><button onClick={sendLink}>发送登录链接</button>{authNotice && <small>{authNotice}</small>}</section></main>
